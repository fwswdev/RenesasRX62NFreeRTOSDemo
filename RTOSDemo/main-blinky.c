...
static void prvQueueBlinkOtherLED(void *pvParameters)
    {
    TickType_t xNextWakeTime;
    LedBlinkDataT *ptrLedBlinkData = (LedBlinkDataT*) pvParameters;
    /* Initialise xNextWakeTime - this only needs to be done once. */
    xNextWakeTime = xTaskGetTickCount();
	...
	/* Place this task in the blocked state until it is time to run again.
	 ...
	 to ms. */
	vTaskDelayUntil(&xNextWakeTime, getRtosDelay(ptrLedBlinkData->delay));
	vParTestToggleLED(ptrLedBlinkData->portNum);
